...
export async function onRequestGet(context) {
  ...
  const appUrl = url.origin + '/app/settings/integrations'
  ...
  try {
    ...
    const saveRes = await fetch(supabaseUrl + '/rest/v1/org_integrations', { method: 'POST', headers: { 'Content-Type': 'application/json', 'apikey': serviceKey, 'Authorization': 'Bearer ' + serviceKey, 'Prefer': 'return=minimal' }, body: JSON.stringify({ user_id: uid, integration_id: provider, status: 'active', access_token: tokens.access_token || null, refresh_token: tokens.refresh_token || null, token_expires_at: tokens.expires_in ? new Date(Date.now() + tokens.expires_in * 1000).toISOString() : null, provider_data: JSON.stringify(tokens), config: {}, connected_at: new Date().toISOString() }) })
