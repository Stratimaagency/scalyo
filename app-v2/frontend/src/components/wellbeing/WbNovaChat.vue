  ...
  <div class="nova-container">
    ...
    <div class="nova-messages" ref="messagesEl">
      <div v-for="msg in store.novaMessages" :key="msg.id" class="nova-msg" :class="msg.role">
        <div class="nova-msg-av">{{ msg.role === 'user' ? '\u{1F464}' : '\u{1F49A}' }}</div>
      ...
      <div v-if="store.novaLoading" class="nova-msg assistant">
        <div class="nova-msg-av">\u{1F49A}</div>
    ...
    <div class="nova-input">
      ...
      <button class="nova-send" @click="send(input)" :disabled="!input.trim() || store.novaLoading">\u{27A1}</button>
    </div>
  </div>
</template>
