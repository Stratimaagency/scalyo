// Salesforce — connexion via identifiants
...
export async function testConnection(config) {
  ...
  return { ok: true, message: `Salesforce configure pour ${config.email}` }
}

export async function sync(config) {
  if (!config.email || !config.password) throw new Error('Identifiants manquants')
  return { ok: true, message: 'Salesforce connecte. Synchronisation configuree.' }
}
