  ...
  <div class="fade-in" style="max-width: 1100px; padding: 26px 30px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 26px;">
      <div>
        ...
        <p style="color: #2d3748; font-size: 13px;">{{ company?.name }} · {{ todayFormatted }}</p>
      </div>
      ...
      <div class="card card-lift" style="padding: 20px;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px;">
          <div>
            ...
            <div style="font-size: 12px; color: #2d3748;">{{ roadmapPhase }}</div>
          </div>
        ...
        <div style="margin-top: 14px; display: flex; flex-direction: column; gap: 6px;">
          ...
          <p v-if="!roadmapTotal" style="font-size: 13px; color: #2d3748; text-align: center; padding: 8px;">
            {{ t('noRoadmapItems') }}
          </p>
        </div>
      </div>
      ...
      <div class="card" style="padding: 20px;">
        <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 14px;">
          ...
          <router-link :to="{ name: 'wellbeing' }" class="btn-base"
            style="font-size: 13px; padding: 5px 12px; border-radius: 20px; background: var(--surface); border: 1px solid var(--border); color: #2d3748; text-decoration: none;">
            {{ t('detailBtn') }} →
          </router-link>
        </div>
        ...
        <p v-if="!teamMembers.length" style="font-size: 12px; color: #2d3748; text-align: center; padding: 8px 0;">
          {{ t('noTeamMembers') }}
        </p>
      </div>
    </div>
        ...
        <div v-for="s in csmStats" :key="s.csm"
          ...
          <div style="display: flex; flex-direction: column; gap: 5px;">
            <div style="display: flex; justify-content: space-between; font-size: 12px;">
              <span style="color: #2d3748; display: flex; align-items: center; gap: 4px;"><ScalyoIcon name="briefcase" :size="13" /> {{ t('accounts') }}</span>
              <span style="font-weight: 700;">{{ s.count }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 12px;">
              <span style="color: #2d3748; display: flex; align-items: center; gap: 4px;"><ScalyoIcon name="money" :size="13" /> ARR</span>
              <span style="font-weight: 700; color: var(--teal);">{{ fmtCurrency(s.arr) }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; font-size: 12px;">
              <span style="color: #2d3748; display: flex; align-items: center; gap: 4px;"><ScalyoIcon name="heart" :size="13" /> {{ t('health') }}</span>
            ...
            <div v-if="s.crit > 0" style="display: flex; justify-content: space-between; font-size: 12px;">
              <span style="color: #2d3748; display: flex; align-items: center; gap: 4px;"><ScalyoIcon name="siren" :size="13" /> {{ t('filterCritical') }}</span>
    ...
    <div v-if="accounts.length" class="card" style="margin-top: 14px; padding: 18px;">
      <details>
        ...
        <p style="font-size: 12px; color: #2d3748; line-height: 1.7; margin-top: 10px;">
          {{ t('healthMethodDesc') }}
        </p>
      </details>
    </div>
