<template>
  <div class="fade-in">
    <div v-if="loading" style="display: flex; justify-content: center; align-items: center; padding: 60px 0; color: #2d3748; font-size: 14px">
      {{ t('loading') }}
    </div>
    <template v-else>
    ...
    <template v-if="tab === 'executive'">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 10px; margin-bottom: 20px;">
        <div class="card" style="padding: 16px; text-align: center;">
          <div style="font-size: 12px; text-transform: uppercase; color: #2d3748; font-weight: 700; margin-bottom: 4px;">ARR Total</div>
        ...
        <div class="card" style="padding: 16px; text-align: center;">
          <div style="font-size: 12px; text-transform: uppercase; color: #2d3748; font-weight: 700; margin-bottom: 4px;">MRR</div>
        ...
        <div class="card" style="padding: 16px; text-align: center;">
          <div style="font-size: 12px; text-transform: uppercase; color: #2d3748; font-weight: 700; margin-bottom: 4px;">NRR</div>
        ...
        <div class="card" style="padding: 16px; text-align: center;">
          <div style="font-size: 12px; text-transform: uppercase; color: #2d3748; font-weight: 700; margin-bottom: 4px;">GRR</div>
        ...
        <div class="card" style="padding: 16px; text-align: center;">
          <div style="font-size: 12px; text-transform: uppercase; color: #2d3748; font-weight: 700; margin-bottom: 4px;">NPS</div>
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px; margin-bottom: 16px;">
        <div class="card" style="padding: 16px;">
          <div style="font-size: 13px; color: #2d3748; font-weight: 700; text-transform: uppercase;">ARPU</div>
          <div style="font-size: 20px; font-weight: 900; font-family: 'JetBrains Mono', monospace; color: var(--teal);">{{ fmtKpiCurrency(arpu) }}</div>
          <div style="font-size: 13px; color: #2d3748;">{{ t('copilArpuDesc') }}</div>
        </div>
        <div class="card" style="padding: 16px;">
          <div style="font-size: 13px; color: #2d3748; font-weight: 700; text-transform: uppercase;">{{ t('copilChurnRate') }}</div>
          ...
          <div style="font-size: 13px; color: #2d3748;">{{ t('copilLogoChurn') }}</div>
        </div>
        <div class="card" style="padding: 16px;">
          <div style="font-size: 13px; color: #2d3748; font-weight: 700; text-transform: uppercase;">{{ t('copilCsEfficiency') }}</div>
          ...
          <div style="font-size: 13px; color: #2d3748;">{{ t('copilArrPerCsm') }}</div>
        </div>
      </div>
    </template>
      ...
      <div class="card mb-md" style="padding: 20px">
        ...
        <div style="margin-bottom: 16px; position: relative;">
          ...
          <div v-if="selectedClient" style="margin-top: 8px; padding: 8px 14px; background: var(--tealBg); border: 1px solid var(--tealBorder); border-radius: 8px; display: flex; align-items: center; justify-content: space-between;">
            <span style="font-weight: 700; font-size: 13px;">{{ selectedClient.name }} <span style="font-size: 12px; color: #2d3748; margin-left: 8px;">{{ selectedClient.csm || '—' }}</span></span>
          ...
          <div v-if="showClientDropdown && clientResults.length > 0" style="position: absolute; z-index: 100; top: 100%; left: 0; right: 0; background: var(--bg1); border: 1px solid var(--border); border-radius: 10px; box-shadow: 0 8px 24px rgba(0,0,0,0.2); max-height: 240px; overflow-y: auto; margin-top: 4px;">
            <div v-for="acc in clientResults" :key="acc.id" @click="selectClient(acc)" style="padding: 10px 14px; cursor: pointer; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between;" @mouseenter="$event.target.style.background='var(--surface)'" @mouseleave="$event.target.style.background='transparent'">
              <div>
                ...
                <div style="font-size: 13px; color: #2d3748;">{{ acc.csm || '—' }} · Health {{ acc.health || 0 }}/100</div>
              </div>
    ...
    <template v-if="tab === 'team'">
      <div class="card mb-md" style="padding: 20px">
        ...
        <div v-if="!csmPerformance.length" style="text-align: center; padding: 20px; color: #2d3748; font-size: 13px;">{{ t('copilNoTeamData') }}</div>
      </div>
    </template>
          ...
          <div v-for="acc in upcomingRenewals" :key="acc.id" style="display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; border-radius: 8px; border: 1px solid var(--border);">
            <div style="display: flex; align-items: center; gap: 10px;">
              ...
              <div>
                ...
                <div style="font-size: 13px; color: #2d3748;">{{ acc.csm || '—' }} · Health {{ acc.health || 0 }}</div>
              </div>
            </div>
            <div style="text-align: right;">
              <div style="font-weight: 700; font-family: 'JetBrains Mono', monospace;">{{ fmtKpiCurrency(acc.arr || 0) }}</div>
              <div style="font-size: 13px; color: #2d3748;">{{ acc.renewal || '—' }}</div>
            </div>
          </div>
        </div>
        <div v-else style="text-align: center; padding: 20px; color: #2d3748; font-size: 13px;">{{ t('copilNoRenewals') }}</div>
      </div>
    </template>
    ...
    <template v-if="tab === 'custom'">
      <div class="flex-between mb-md">
        <div>
          ...
          <p style="font-size: 13px; color: #2d3748">{{ t('kpiCustomDesc') }}</p>
        </div>
        ...
        <AppCard v-for="(kpi, i) in customKpis" :key="i">
          <div class="flex-between">
            <div>
              ...
              <div style="font-size: 12px; color: #2d3748">{{ kpi.unit || '' }}</div>
            </div>
            <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <div v-if="kpi.goal && kpi.goal > 0" style="font-size: 13px; color: #2d3748; margin-top: 6px">
