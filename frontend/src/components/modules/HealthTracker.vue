<template>
  <div class="mod-page fade-in">
    ...
    <div class="mod-card" v-if="clientsStore.clients.length">
      <h3 class="mod-section-title">📊 {{ t('htDistribution') }}</h3>
...
function statusLabel(s) { return s === 'healthy' ? t('htStatusHealthy') : s === 'neutral' ? t('htStatusNeutral') : t('htStatusAtRisk') }
...
const distChartOpts = computed(() => ({
  ...
  xaxis: { categories: [t('htRiskHigh'), t('htRiskMedium'), t('htRiskLow')] },
  legend: { show: false },
}))
